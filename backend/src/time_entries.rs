...
fn duration_min(start: &str, end: &str) -> AppResult<i64> {
    ...
    if e <= b { return Err(AppError::BadRequest("End time must be after start time.".into())); }
    Ok((e - b).num_minutes())
}

#[derive(Deserialize)]
pub struct RangeQuery { pub from: Option<NaiveDate>, pub to: Option<NaiveDate>, pub user_id: Option<i64>, pub status: Option<String> }

pub async fn list(State(s): State<AppState>, u: User, Query(q): Query<RangeQuery>) -> AppResult<Json<Vec<TimeEntry>>> {
    let mut sql = String::from("SELECT * FROM time_entries WHERE user_id=?");
    if q.from.is_some() { sql += " AND entry_date >= ?"; }
    if q.to.is_some() { sql += " AND entry_date <= ?"; }
...
pub async fn list_all(State(s): State<AppState>, u: User, Query(q): Query<RangeQuery>) -> AppResult<Json<Vec<TimeEntry>>> {
    ...
    if let Some(v) = q.from { qx = qx.bind(v); }
    if let Some(v) = q.to { qx = qx.bind(v); }
    if let Some(v) = q.user_id { qx = qx.bind(v); }
...
async fn validate(pool: &sqlx::SqlitePool, user_id: i64, te: &NewTimeEntry, exclude_id: Option<i64>) -> AppResult<()> {
    if te.entry_date > chrono::Local::now().date_naive() {
        return Err(AppError::BadRequest("Entries in the future are not allowed.".into()));
    }
    ...
    ).bind(user_id).bind(te.entry_date).fetch_all(pool).await?;
...
pub async fn create(State(s): State<AppState>, u: User, Json(b): Json<NewTimeEntry>) -> AppResult<Json<TimeEntry>> {
    ...
    let z: TimeEntry = sqlx::query_as("SELECT * FROM time_entries WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    audit::log(&s.pool, u.id, "created", "time_entries", id, None, Some(serde_json::to_value(&z).unwrap())).await;
    Ok(Json(z))
}

pub async fn update(State(s): State<AppState>, u: User, Path(id): Path<i64>, Json(b): Json<NewTimeEntry>) -> AppResult<Json<TimeEntry>> {
    let prev: TimeEntry = sqlx::query_as("SELECT * FROM time_entries WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    let admin_correction = u.is_admin() && prev.status == "approved";
    if !admin_correction {
        if prev.user_id != u.id { return Err(AppError::Forbidden); }
        if prev.status != "draft" { return Err(AppError::BadRequest("Only drafts can be edited directly. Please file a change request.".into())); }
    }
    ...
    sqlx::query("UPDATE time_entries SET entry_date=?, start_time=?, end_time=?, category_id=?, comment=?, updated_at=CURRENT_TIMESTAMP WHERE id=?")
        ...
        .execute(&s.pool).await?;
    let next: TimeEntry = sqlx::query_as("SELECT * FROM time_entries WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    audit::log(&s.pool, u.id, "updated", "time_entries", id, Some(serde_json::to_value(&prev).unwrap()), Some(serde_json::to_value(&next).unwrap())).await;
    Ok(Json(next))
}

pub async fn delete(State(s): State<AppState>, u: User, Path(id): Path<i64>) -> AppResult<Json<serde_json::Value>> {
    let z: TimeEntry = sqlx::query_as("SELECT * FROM time_entries WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    if z.user_id != u.id { return Err(AppError::Forbidden); }
    if z.status != "draft" { return Err(AppError::BadRequest("Only drafts can be deleted.".into())); }
    sqlx::query("DELETE FROM time_entries WHERE id=?").bind(id).execute(&s.pool).await?;
    audit::log(&s.pool, u.id, "deleted", "time_entries", id, Some(serde_json::to_value(&z).unwrap()), None).await;
    Ok(Json(serde_json::json!({"ok": true})))
}

#[derive(Deserialize)]
pub struct IdsBody { pub ids: Vec<i64> }

pub async fn submit(State(s): State<AppState>, u: User, Json(b): Json<IdsBody>) -> AppResult<Json<serde_json::Value>> {
    for id in &b.ids {
        let z: TimeEntry = sqlx::query_as("SELECT * FROM time_entries WHERE id=?").bind(id).fetch_one(&s.pool).await?;
        if z.user_id != u.id { return Err(AppError::Forbidden); }
        if z.status != "draft" { continue; }
        sqlx::query("UPDATE time_entries SET status='submitted', submitted_at=CURRENT_TIMESTAMP WHERE id=?")
            .bind(id).execute(&s.pool).await?;
        audit::log(&s.pool, u.id, "status_changed", "time_entries", *id, Some(serde_json::json!({"status":"draft"})), Some(serde_json::json!({"status":"submitted"}))).await;
    }
...
pub async fn approve(State(s): State<AppState>, u: User, Path(id): Path<i64>) -> AppResult<Json<serde_json::Value>> {
    if !u.is_lead() { return Err(AppError::Forbidden); }
    let z: TimeEntry = sqlx::query_as("SELECT * FROM time_entries WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    if z.user_id == u.id && !u.is_admin() { return Err(AppError::Forbidden); }
...
pub async fn reject(State(s): State<AppState>, u: User, Path(id): Path<i64>, Json(b): Json<RejectBody>) -> AppResult<Json<serde_json::Value>> {
    ...
    audit::log(&s.pool, u.id, "rejected", "time_entries", id, None, Some(serde_json::json!({"reason": b.reason}))).await;
    Ok(Json(serde_json::json!({"ok":true})))
}

pub async fn batch_approve(State(s): State<AppState>, u: User, Json(b): Json<IdsBody>) -> AppResult<Json<serde_json::Value>> {
    if !u.is_lead() { return Err(AppError::Forbidden); }
    let mut count = 0;
    for id in &b.ids {
        let z: Option<TimeEntry> = sqlx::query_as("SELECT * FROM time_entries WHERE id=? AND status='submitted'").bind(id).fetch_optional(&s.pool).await?;
        let Some(z) = z else { continue };
        if z.user_id == u.id && !u.is_admin() { continue; }
        sqlx::query("UPDATE time_entries SET status='approved', reviewed_by=?, reviewed_at=CURRENT_TIMESTAMP WHERE id=?")
            .bind(u.id).bind(id).execute(&s.pool).await?;
        audit::log(&s.pool, u.id, "approved", "time_entries", *id, None, None).await;
        count += 1;
