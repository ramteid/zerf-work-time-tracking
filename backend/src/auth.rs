...
impl User {
    ...
    pub fn is_lead(&self) -> bool { self.role == "team_lead" || self.role == "admin" }
}
...
pub fn verify_password(password: &str, hash: &str) -> bool {
    if let Ok(parsed) = PasswordHash::new(hash) {
        argon2_instance().verify_password(password.as_bytes(), &parsed).is_ok()
    } else { false }
}
...
pub fn validate_password_strength(pw: &str) -> AppResult<()> {
    ...
    if pw.len() > 256 {
        return Err(AppError::BadRequest("Password is too long (max 256 chars).".into()));
    }
    let has_lower = pw.chars().any(|c| c.is_ascii_lowercase());
    let has_upper = pw.chars().any(|c| c.is_ascii_uppercase());
    let has_digit = pw.chars().any(|c| c.is_ascii_digit());
    let has_other = pw.chars().any(|c| !c.is_ascii_alphanumeric());
    let classes = [has_lower, has_upper, has_digit, has_other].iter().filter(|x| **x).count();
    if classes < 3 {
...
pub async fn login(State(s): State<AppState>, headers: axum::http::HeaderMap, Json(req): Json<LoginReq>) -> AppResult<Response> {
    ...
    ).bind(&email).bind(since).fetch_one(&s.pool).await?;
    ...
    let ok = match &user {
        ...
        None => { let _ = verify_password(&req.password, dummy); false }
    };
    sqlx::query("INSERT INTO login_attempts(email, success) VALUES (?, ?)")
        .bind(&email).bind(ok).execute(&s.pool).await?;
    let user = user.ok_or_else(|| AppError::BadRequest("Invalid email or password.".into()))?;
    if !ok { return Err(AppError::BadRequest("Invalid email or password.".into())); }
    ...
    sqlx::query("INSERT INTO sessions(token, user_id, csrf_token) VALUES (?, ?, ?)")
        .bind(&token).bind(user.id).bind(&csrf).execute(&s.pool).await?;
    ...
    sqlx::query("DELETE FROM login_attempts WHERE email = ? AND success = 0")
        .bind(&email).execute(&s.pool).await.ok();
...
pub async fn logout(State(s): State<AppState>, req: Request) -> AppResult<Response> {
    if let Some(token) = extract_token(&req) {
        sqlx::query("DELETE FROM sessions WHERE token = ?").bind(&token).execute(&s.pool).await?;
    }
...
pub async fn me(State(s): State<AppState>, user: User, req: Request) -> AppResult<Json<serde_json::Value>> {
    ...
    let csrf: Option<String> = sqlx::query_scalar("SELECT csrf_token FROM sessions WHERE token = ?")
        .bind(&token).fetch_optional(&s.pool).await?;
...
#[derive(Deserialize)]
pub struct PasswordReq { pub current_password: Option<String>, pub new_password: String }

pub async fn change_password(State(s): State<AppState>, user: User, headers: axum::http::HeaderMap, Json(body): Json<PasswordReq>) -> AppResult<Json<serde_json::Value>> {
    let req_token: Option<String> = headers.get(header::COOKIE).and_then(|v| v.to_str().ok()).and_then(extract_token_from_cookie_str);
    if !user.must_change_password {
        let cur = body.current_password.as_deref()
    ...
    sqlx::query("UPDATE users SET password_hash=?, must_change_password=0 WHERE id=?")
        .bind(h).bind(user.id).execute(&mut *tx).await?;
    ...
    sqlx::query("DELETE FROM sessions WHERE user_id=? AND token != ?")
        .bind(user.id).bind(&cur_token).execute(&mut *tx).await?;
    tx.commit().await?;
...
fn enforce_same_origin_headers(headers: &axum::http::HeaderMap, s: &AppState) -> AppResult<()> {
    if !s.cfg.enforce_origin { return Ok(()); }
    ...
    let matches = |val: &str| allowed.iter().any(|a| val == a || val.starts_with(&format!("{a}/")));
...
/// is defence-in-depth.
async fn enforce_csrf(parts: &axum::http::request::Parts, s: &AppState, csrf_token: &str) -> AppResult<()> {
    if matches!(parts.method, Method::GET | Method::HEAD | Method::OPTIONS) {
        return Ok(());
    }
    enforce_same_origin_headers(&parts.headers, s)?;
    if !s.cfg.enforce_csrf { return Ok(()); }
    let header_token = parts.headers.get("x-csrf-token").and_then(|v| v.to_str().ok()).unwrap_or("");
    if header_token.is_empty() || header_token.as_bytes().ct_eq(csrf_token.as_bytes()).unwrap_u8() == 0 {
        return Err(AppError::Forbidden);
    }
    Ok(())
}

pub async fn auth_middleware(State(s): State<AppState>, req: Request, next: Next) -> Result<Response, AppError> {
    ...
    sqlx::query("UPDATE sessions SET last_active_at=CURRENT_TIMESTAMP WHERE token=?")
        .bind(&token).execute(&s.pool).await?;
    let user: User = sqlx::query_as("SELECT * FROM users WHERE id=? AND active=1")
        .bind(uid).fetch_optional(&s.pool).await?
        .ok_or(AppError::Unauthorized)?;
...
impl<S> axum::extract::FromRequestParts<S> for User where S: Send + Sync {
    type Rejection = AppError;
    async fn from_request_parts(parts: &mut axum::http::request::Parts, _: &S) -> Result<Self, Self::Rejection> {
        parts.extensions.get::<User>().cloned().ok_or(AppError::Unauthorized)
    }
}
...
pub async fn cleanup_loop(pool: sqlx::SqlitePool) {
    ...
    loop {
        interval.tick().await;
        let _ = sqlx::query(
            "DELETE FROM sessions WHERE last_active_at < datetime('now','-1 day')"
        ).execute(&pool).await;
        let _ = sqlx::query(
