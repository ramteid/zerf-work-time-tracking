...
pub async fn list(State(s): State<AppState>, u: User) -> AppResult<Json<Vec<User>>> {
    ...
    let r = sqlx::query_as::<_, User>("SELECT * FROM users ORDER BY last_name, first_name").fetch_all(&s.pool).await?;
    Ok(Json(r))
}

pub async fn get_one(State(s): State<AppState>, u: User, Path(id): Path<i64>) -> AppResult<Json<User>> {
    if !u.is_lead() && u.id != id { return Err(AppError::Forbidden); }
    Ok(Json(sqlx::query_as::<_, User>("SELECT * FROM users WHERE id=?").bind(id).fetch_one(&s.pool).await?))
}
...
#[derive(Serialize)]
pub struct CreateResponse { pub user: User, pub temporary_password: Option<String> }

pub async fn create(State(s): State<AppState>, u: User, Json(b): Json<NewUser>) -> AppResult<Json<CreateResponse>> {
    ...
    let (password, temp) = match b.password {
        Some(p) if !p.is_empty() => { validate_password_strength(&p)?; (p, None) },
        _ => { let t = generate_password(); (t.clone(), Some(t)) }
    };
    ...
    let user: User = sqlx::query_as("SELECT * FROM users WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    audit::log(&s.pool, u.id, "created", "users", id, None, Some(serde_json::to_value(&user).unwrap())).await;
    Ok(Json(CreateResponse { user, temporary_password: temp }))
}
...
pub async fn update(State(s): State<AppState>, u: User, Path(id): Path<i64>, Json(b): Json<UpdateUser>) -> AppResult<Json<User>> {
    if !u.is_admin() { return Err(AppError::Forbidden); }
    let prev: User = sqlx::query_as("SELECT * FROM users WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    sqlx::query("UPDATE users SET email=COALESCE(?,email), first_name=COALESCE(?,first_name), last_name=COALESCE(?,last_name), role=COALESCE(?,role), weekly_hours=COALESCE(?,weekly_hours), annual_leave_days=COALESCE(?,annual_leave_days), start_date=COALESCE(?,start_date), active=COALESCE(?,active) WHERE id=?")
        ...
        .execute(&s.pool).await?;
    let next: User = sqlx::query_as("SELECT * FROM users WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    audit::log(&s.pool, u.id, "updated", "users", id, Some(serde_json::to_value(&prev).unwrap()), Some(serde_json::to_value(&next).unwrap())).await;
    Ok(Json(next))
}

pub async fn deactivate(State(s): State<AppState>, u: User, Path(id): Path<i64>) -> AppResult<Json<serde_json::Value>> {
    if !u.is_admin() { return Err(AppError::Forbidden); }
    if id == u.id { return Err(AppError::BadRequest("You cannot deactivate yourself.".into())); }
    let mut tx = s.pool.begin().await?;
    sqlx::query("UPDATE users SET active=0 WHERE id=?").bind(id).execute(&mut *tx).await?;
...
pub async fn reset_password(State(s): State<AppState>, u: User, Path(id): Path<i64>) -> AppResult<Json<serde_json::Value>> {
    if !u.is_admin() { return Err(AppError::Forbidden); }
...
pub fn generate_password() -> String {
    ...
    let mut rng = OsRng;
    let mut out: Vec<u8> = pools.iter().map(|p| {
        let mut buf = [0u8; 1]; rng.fill_bytes(&mut buf);
