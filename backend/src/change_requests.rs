...
pub async fn list(State(s): State<AppState>, u: User) -> AppResult<Json<Vec<ChangeRequest>>> {
    Ok(Json(sqlx::query_as::<_, ChangeRequest>("SELECT * FROM change_requests WHERE user_id=? ORDER BY created_at DESC")
        .bind(u.id).fetch_all(&s.pool).await?))
}
...
pub async fn list_all(State(s): State<AppState>, u: User) -> AppResult<Json<Vec<ChangeRequest>>> {
    ...
    Ok(Json(sqlx::query_as::<_, ChangeRequest>("SELECT * FROM change_requests WHERE status='open' ORDER BY created_at").fetch_all(&s.pool).await?))
}
...
pub async fn create(State(s): State<AppState>, u: User, Json(b): Json<NewChangeRequest>) -> AppResult<Json<ChangeRequest>> {
    ...
    let z: (i64, String) = sqlx::query_as("SELECT user_id, status FROM time_entries WHERE id=?")
        .bind(b.time_entry_id).fetch_one(&s.pool).await?;
    if z.0 != u.id { return Err(AppError::Forbidden); }
    if z.1 == "draft" { return Err(AppError::BadRequest("Edit drafts directly.".into())); }
    ...
    let a: ChangeRequest = sqlx::query_as("SELECT * FROM change_requests WHERE id=?").bind(id).fetch_one(&s.pool).await?;
    audit::log(&s.pool, u.id, "created", "change_requests", id, None, Some(serde_json::to_value(&a).unwrap())).await;
    Ok(Json(a))
}

pub async fn approve(State(s): State<AppState>, u: User, Path(id): Path<i64>) -> AppResult<Json<serde_json::Value>> {
    if !u.is_lead() { return Err(AppError::Forbidden); }
    let a: ChangeRequest = sqlx::query_as("SELECT * FROM change_requests WHERE id=? AND status='open'").bind(id).fetch_one(&s.pool).await?;
...
pub async fn reject(State(s): State<AppState>, u: User, Path(id): Path<i64>, Json(b): Json<RejectBody>) -> AppResult<Json<serde_json::Value>> {
    ...
    audit::log(&s.pool, u.id, "rejected", "change_requests", id, None, Some(serde_json::json!({"reason": b.reason}))).await;
    Ok(Json(serde_json::json!({"ok":true})))
}
